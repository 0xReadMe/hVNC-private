...
HOOK_FUNCTION hook_user32_EnableIAMAccessWin80 = 
	{"user32.dll", HF_ORDINAL, "EnableIAMAccess", 2510, &my_EnableIAMAccessWin80, NULL, NULL};

// win81
HOOK_FUNCTION hook_user32_EnableIAMAccessWin81 = 
	{"user32.dll", HF_ORDINAL, "EnableIAMAccess", 2510, &my_EnableIAMAccessWin81, NULL, NULL};

DECLARE_NULL_HOOK_ORDINAL(AcquireIAMKey, 2509);
DECLARE_NULL_HOOK_ORDINAL(EnableIAMAccessWin80, 2510);

HOOK_FUNCTION hook_null_EnableIAMAccessWin81 = 
	{NULL, HF_ORDINAL, "EnableIAMAccess", 2510, &my_EnableIAMAccessWin81, NULL, NULL};

DECLARE_U32_HOOK(MsgWaitForMultipleObjectsEx);
DECLARE_KERNELBASE_HOOK(CloseHandle);
...
// win 8.0
// on win 8.0 EnableIAMAccess has 2 arguments, on win 8.1 it has 3 arguments
static HOOK_DESCRIPTOR g_WndIatHooks80[] =
{
	DEFINE_NULL_IAT_HOOK(EnableIAMAccessWin80),
};

static HOOK_DESCRIPTOR g_WndExpHooks80[] = 
{
	DEFINE_U32_EXP_HOOK(EnableIAMAccessWin80),
};

// win 8.1
// on win 8.0 EnableIAMAccess has 2 arguments, on win 8.1 it has 3 arguments
// also trick with MsgWaitForMultipleObjectsEx does not help anymore, 
// need another hook to terminate f'n thread
static HOOK_DESCRIPTOR g_WndIatHooks81[] =
{
	{&hook_null_EnableIAMAccessWin81, NULL, HOOK_TYPE_IAT},
	//{&hook_null_PsmRegisterKeyNotification, NULL, HOOK_TYPE_IAT},
	DEFINE_KERNELBASE_IAT_HOOK(CloseHandle),
};

static HOOK_DESCRIPTOR g_WndExpHooks81[] = 
{
	{&hook_user32_EnableIAMAccessWin81, NULL, HOOK_TYPE_EXPORT},
	//{&hook_twinapi_PsmRegisterKeyNotification, NULL, HOOK_TYPE_EXPORT}
	DEFINE_KERNELBASE_EXP_HOOK(CloseHandle),
};
...
static PVOID g_pImShellCaller = NULL;
static DWORD g_dwImShellCountDown = 0;

#define IM_SHELL_START_TIMEOUT        5000 // 5 sec
...
{
	for (unsigned int i = 0; g_VirtGuids[i].DllName != NULL; i++)
	{
...
HRESULT __stdcall my_CoCreateInstance(REFCLSID rclsid, LPUNKNOWN pUnkOuter, DWORD dwClsContext, REFIID riid, LPVOID * ppv)
{
	...
	//else
	//{
	//	DbgPrint("clsid={%08X-%04X-%04X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X} riid={%08X-%04X-%04X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X}\n",
	//		rclsid->Data1, rclsid->Data2, rclsid->Data3,
	//		rclsid->Data4[0], rclsid->Data4[1], rclsid->Data4[2], rclsid->Data4[3], rclsid->Data4[4], rclsid->Data4[5], rclsid->Data4[6], rclsid->Data4[7],
	...
	{
		ptr_CoCreateInstance Func_CoCreateInstance = IsWIN8andAbove() ? (ptr_CoCreateInstance)hook_combase_CoCreateInstance.Original : DEFINE_OLE32_PROC(CoCreateInstance);
...
HRESULT __stdcall my_CoCreateInstanceEx(REFCLSID rclsid, IUnknown * punkOuter, DWORD dwClsCtx, COSERVERINFO * pServerInfo, ULONG cmq, MULTI_QI * pResults)
{
	...
	{
		ptr_CoCreateInstanceEx Func_CoCreateInstanceEx = IsWIN8andAbove() ? (ptr_CoCreateInstanceEx)hook_combase_CoCreateInstanceEx.Original : DEFINE_OLE32_PROC(CoCreateInstanceEx);
...
HRESULT __stdcall my_CoGetClassObject(REFCLSID rclsid, DWORD dwClsContext, COSERVERINFO * pServerInfo, REFIID riid, LPVOID * ppv)
{
	...
	{
		ptr_CoGetClassObject Func_CoGetClassObject = IsWIN8andAbove() ? (ptr_CoGetClassObject)hook_combase_CoGetClassObject.Original : DEFINE_OLE32_PROC(CoGetClassObject);
...
HRESULT __stdcall my_CoRegisterClassObject(REFCLSID rclsid, IUnknown * pUnk, DWORD dwClsContext, DWORD flags, LPDWORD lpdwRegister)
{
	...
	{
		ptr_CoRegisterClassObject Func_CoRegisterClassObject = IsWIN8andAbove() ? (ptr_CoRegisterClassObject)hook_combase_CoRegisterClassObject.Original : DEFINE_OLE32_PROC(CoRegisterClassObject);
...
DWORD __stdcall my_MsgWaitForMultipleObjectsEx(DWORD nCount, HANDLE *pHandles, DWORD dwMilliseconds, DWORD dwWakeMask, DWORD dwFlags)
{
	DWORD r;
	...
	{
		if (g_pImShellCaller == NULL)
		{
			CHAR FileName[MAX_PATH];
			DWORD Length;

			// dirty hack (identify dll by caller)
			...
			if (Length)
			{
				...
				{
					DbgPrint("IM shell detected at %p\n", ret);
					g_pImShellCaller = ret;
				}
			}
		}

		if (ret && ret == g_pImShellCaller)
		{
			if (g_dwImShellCountDown == 0)
			{
				DbgPrint("Give the IM shell %u sec to initialize\n", IM_SHELL_START_TIMEOUT);
				g_dwImShellCountDown = GetTickCount();
			}

			if (GetTickCount() - g_dwImShellCountDown > IM_SHELL_START_TIMEOUT)
			{
				DbgPrint("Terminate the IM shell\n");
	...
	// call the original
	r =	DEFINE_U32_PROC(MsgWaitForMultipleObjectsEx)(nCount, pHandles, dwMilliseconds, dwWakeMask, dwFlags);
	LEAVE_HOOK();
	return r;
}

// the same idea for win 10.0
DWORD __stdcall my_CloseHandle(HANDLE hObject)
{
	DWORD r;
	PVOID ret = _ReturnAddress();
	...
	if (g_bImShellIsTerminated == FALSE)
	{
		...
		{
			CHAR FileName[MAX_PATH];
			// identify dll by caller
			if (GetMappedFileNameA(GetCurrentProcess(), ret, FileName, MAX_PATH))
			{
				PathStripPath(FileName);
				if (lstrcmpiA(FileName, "windows.immersiveshell.serviceprovider.dll") == 0)
				{
		...
		{
			DbgPrint("Give the IM shell %u sec to initialize\n", IM_SHELL_START_TIMEOUT);
			SleepEx(IM_SHELL_START_TIMEOUT, TRUE);
			DbgPrint("Terminate the IM shell\n");
			g_bImShellIsTerminated = TRUE;
			LEAVE_HOOK();
	...
	// call the original
	r = DEFINE_KERNELBASE_PROC(CloseHandle)(hObject);
	LEAVE_HOOK();
	return r;
}

//// twinapi.appcore!PsmRegisterKeyNotification registers explorer in some service
//// we just disable it
...
WINERROR ShellHookActivate(void)
{
	...
	PHOOK_DESCRIPTOR ExportHooks = NULL;
	PHOOK_DESCRIPTOR IatHooks = NULL;
	LONG NumberIatHooks;
	...
	// system specific
	if (SystemMajor < 6 || (SystemMajor == 6 && SystemMinor <= 1)) // Windows 7 and below
	{
		DbgPrint("ShellHookActivate 4 win7-\n");
	...
	else // Windows 8 and higher
	{
		DbgPrint("ShellHookActivate 4 win8+\n");
		...
		if ((SystemMajor > 6) || (SystemMinor > 2)) // win 8.1 || 10.0
		{
			DbgPrint("ShellHookActivate 4 win8.1/10.0\n");

			// special workaround for win 8.1
			// we need to hook twinui_appcore exports but this dll is loaded later by com, so we load and hook it now
		...
		{
			DbgPrint("ShellHookActivate 4 win8.0\n");
	...
	DbgPrint("SetMultipleDllHooks main\n");
	Status = SetMultipleDllHooks(IatHooks, NumberIatHooks, ExportHooks, NumberExportHooks);
	if (Status == NO_ERROR && ExportHooks8 && IatHooks8)
	{
		DbgPrint("SetMultipleDllHooks win8 specific\n");
		Status = SetMultipleDllHooks(IatHooks8, NumberIatHooks8, ExportHooks8, NumberExportHooks8);
	}

	DbgPrint("SetMultipleDllHooks %s\n", (Status == NO_ERROR) ? "ok" : "FAIL");
	return Status;
}
