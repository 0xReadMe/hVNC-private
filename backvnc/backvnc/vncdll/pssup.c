	...
	)
{
	...
	if (!g_ZwWow64QueryInformationProcess64)
		g_ZwWow64QueryInformationProcess64 = (FUNC_ZwWow64QueryInformationProcess64)GetProcAddress(GetModuleHandleW(wczNtdll), "ZwWow64QueryInformationProcess64");
	...
	)
{
	...
	if (ModuleName && (NewName = AppAlloc(lstrlen(ModuleName) + 3*sizeof(_TCHAR))))
	{
	...
	)
{
	...
	if (g_IsWow64ProcessPtr == NULL ){
		g_IsWow64ProcessPtr = (LPFN_ISWOW64PROCESS) GetProcAddress(GetModuleHandleW(wczKernel32),"IsWow64Process");
	}
...
{
	if (!g_RedirectWow64)
		g_RedirectWow64 = (FUNC_RedirectWow64)GetProcAddress(GetModuleHandleW(wczKernel32), "Wow64EnableWow64FsRedirection");
...
PVOID	GetLoadLibraryPtr(VOID)
{
	...
	if (!g_LoadLibraryPtr)
	{
		...
		if (pLoadLibrary = PsSupGetRealFunctionAddress(GetModuleHandleW(wczKernel32), szLoadLibraryA))
	...
	)
{
	...
	if (!g_ZwWow64ReadVirtualMemory64)
		g_ZwWow64ReadVirtualMemory64 = (FUNC_ZwWow64ReadVirtualMemory64)GetProcAddress(GetModuleHandleW(wczNtdll), "ZwWow64ReadVirtualMemory64");
	...
	)
{
	...
	HMODULE	hNtdll = GetModuleHandleW(wczNtdll);
...
PPSSUP_NATIVE_POINTERS PsSupResolveNativePointers(VOID)
{
	...
	if (!g_Wow64NativeCallPointers.pZwGetContextThread)
	{
		...
		do
		{
			if (!(g_Wow64NativeCallPointers.pZwGetContextThread = PsSupGetProcessFunctionAddressArch(hCurrentProcess, szNtdll, "ZwGetContextThread")))
				break;

			if (!(g_Wow64NativeCallPointers.pZwSetContextThread = PsSupGetProcessFunctionAddressArch(hCurrentProcess, szNtdll, "ZwSetContextThread")))
				break;
	...
	)
{
	...
	hInst = ShellExecute(0, szOpen, FilePath, ParamStr, NULL, SW_SHOWNORMAL);
	if (*(PULONG)&hInst > 32)
	{
