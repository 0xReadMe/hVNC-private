...
// module: image.c
// $Revision: 162 $
// $Date: 2014-02-14 19:44:06 +0400 (Пт, 14 фев 2014) $
// description: 
	...
	)
{
	...
	{
		DbgPrint("A section of %u bytes mapped to the target process at 0x%p\n", SectionSize, *pSectionBase);
	}
	else
	{
		DbgPrint("Failed mapping a section to the target process, status 0x%x\n", ntStatus);
	}
	...
	)
{
	...
	else
	{
		...
		DbgPrint("Failed creating an image section of %u bytes, status 0x%x\n", SizeOfSection, ntStatus);
	}
