...
WINERROR AcInjectDll(LPPROCESS_INFORMATION lpProcessInformation, DWORD ProcessCreateFlags, BOOL bInjectImage);
...
#endif
		if (Ret)
			AcInjectDll(lpProcessInformation, Flags, TRUE);
	}
						...
						)
{
	...
	ULONGLONG	pZwProtectVirtualMemory = PsSupGetProcessFunctionAddressArch(hCurrentProcess, szNtdll, "ZwProtectVirtualMemory");
	ULONGLONG	pZwWriteVirtualMemory = PsSupGetProcessFunctionAddressArch(hCurrentProcess, szNtdll, "ZwWriteVirtualMemory");
...
void AcCleanup(void)
{
	// We do not cleanup active dll hooks, because all of them are linked into the application g_HookList, and will be removed together with all other hooks.
}