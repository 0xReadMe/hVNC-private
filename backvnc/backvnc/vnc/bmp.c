	...
	if ((dwPixelsPerLine > 0) && (dwLines > 0))
	{
		...
		//VncLockFrameBuffer( &pSession->SharedSection );
		{
			if ( pSession->SharedSection.pFrameBuffer )
			{
				if ( !bClient )
				{
					lpSrc  = (PUCHAR)pSession->SharedSection.pFrameBuffer + rect.top*dwBytesPerLine + rect.left*bBytesPerPixel;
					lpDest = (PUCHAR)pSession->Desktop.lpIntermedDIB + rect.top*dwBytesPerLine + rect.left*bBytesPerPixel;
				}
				else
				{
					lpDest = (PUCHAR)pSession->SharedSection.pFrameBuffer + rect.top*dwBytesPerLine + rect.left*bBytesPerPixel;
